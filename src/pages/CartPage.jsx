...
function CartPage() {
  ...
  const initialOrderList =
    JSON.parse(localStorage.getItem(`orderList_${cafeId}_${tableId}`)) ||
    location.state?.orderList ||
    []
  ...
  const handlePlaceOrder = async () => {
    try {
      ...
      localStorage.setItem(
        `orderHistory_${cafeId}_${tableId}`,
      ...
      if (result.success) {
        ...
        setError(
          result.message || 'Error placing order. Please try again later.'
        )
        setIsSubmitting(false)
      }
    } catch (error) {
  ...
  const getOrderHistory = () => {
    const orderHistory = JSON.parse(
      localStorage.getItem(`orderHistory_${cafeId}_${tableId}`)
    )
  ...
  return (
    <div>
      <div className="relative bg-user_bg">
        ...
        {!orderPlaced && (
          <div
            className={`font-montserrat-600 px-3 mb-2.5 ${
              orderList.length === 0 ? 'opacity-0' : ''
            }`}
          >
            Your Order
          </div>
        ...
        {error && (
          <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-2 rounded mx-3 mb-3">
            {error}
          </div>
        )}
        ...
        {orderPlaced ? (
          ...
          <>
            <div className="flex flex-col font-montserrat-500 gap-2 px-3 pb-28 h-[70vh] overflow-y-auto">
              {orderList.length === 0 ? (
                <div>
                  <p className="w-full px-auto text-center">
                    Your cart is empty <br /> Please go back and add something
                    to see here.
                  </p>
                  <div>
                    {orderHistory.length !== 0 && (
                      <div className="flex flex-col gap-1 mt-4">
                        <div className="flex justify-between items-center w-full">
                          ...
                          <div
                            onClick={() => setOpenOrderHistory((prev) => !prev)}
                          >
                            <img
                              src={DownArrow}
                              alt="Down Arrow"
                              className={`h-6 w-6 transition-transform duration-300 ${
                                openOrderHistory ? 'rotate-180' : ''
                              }`}
                            />
                          </div>
                        </div>
                        {openOrderHistory && (
                          <div className="px-3 py-2 rounded-2xl shadow-[0_0_18px_rgba(0,0,0,0.15)] bg-user_comp">
                            {orderHistory.map((item, index) => (
                              <div
                                key={index}
                                className="flex flex-col gap-0.5 pb-0.5"
                              >
                                ...
                                {/* Addons Section */}
                                {item.dishAddOns &&
                                  item.dishAddOns.length > 0 && (
                                    <div className="flex gap-1 flex-wrap">
                                      {item.dishAddOns.map((addon, idx) => (
                                        <div
                                          key={idx}
                                          className="rounded-2xl text-xs border-2 px-1 capitalize w-[45%]"
                                        >
                                          {addon.addOnName}
                                        </div>
                                      ))}
                ...
                <div className="flex flex-col">
                  ...
                  <div className="mt-2">
                    <div className="font-montserrat-600 mb-1.5 text-sm">
                      Additional Details
                    </div>
            ...
            {orderList.length > 0 && (
              <div className="absolute flex justify-between items-center z-50 pt-3 pb-6 w-full px-3 font-montserrat-500 shadow-[0_0_18px_rgba(0,0,0,0.3)] bg-user_comp bottom-0">
                ...
                <button
                  ...
                  className={`uppercase ${
                    isSubmitting ? 'bg-gray-400' : 'bg-user_blue'
                  } text-black rounded-xl py-1 px-3 hover:bg-opacity-90 flex items-center`}
                >
                  {isSubmitting ? (
                    <>
